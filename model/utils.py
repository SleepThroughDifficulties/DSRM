...
class Collator:
    ...
    def __call__(self, features):
        ...
        texts, model_inputs, labels = list(zip(*features))
        ...
        return texts, padded_inputs, labels
...
class Huggingface_dataset(Dataset):
    def __init__(
        ...
        if subset == "None" or not subset:
            self.dataset = datasets.load_dataset(self.name)[split]
        else:
            self.dataset = datasets.load_dataset(self.name, subset)[split]

        if subset and not subset == "None":
